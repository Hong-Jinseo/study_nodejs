...
var app = http.createServer(function(request,response){
  ...
  if(pathname === '/'){
    // pathname '/', queryData.id undefined -> 메인화면
    if(queryData.id === undefined){
      fs.readdir('./data', function(error, filelist){
        ...
        var html = template.html(title, list, `<a href="/create">create</a>`, `<h2>${title}</h2><p>${description}</p>`);

        response.writeHead(200);
        response.end(html);
      });

    // pathname '/', queryData.id 존재 -> queryData.id(title) 페이지로 이동
    }else{
      fs.readdir('./data', function(error, filelist){
